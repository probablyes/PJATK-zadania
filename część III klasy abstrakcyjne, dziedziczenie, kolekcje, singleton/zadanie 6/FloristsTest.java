...
public class FloristsTest {
  ...
  static int valueOf(Box box, String color) {
	  double v = 0;
	   for (java.util.Map.Entry<String, Double> entry : PriceList.getInstance().hm.entrySet()){
			for (int j = 0; j < box.getZbior().size(); j++) {
			if(box.getZbior().get(j).getNazwa().equals((entry.getKey()))
					&& (box.getZbior().get(j).getKolor().equals(color))){
				int a = 0;
					for (int i = 0; i < box.getZbior().size(); i++) {
						if(box.getZbior().get(i).getNazwa()==entry.getKey()){
							a = i;
						}
					}
					v = v + (entry.getValue()*(box.getZbior().get(a).getIlosc()));
				}
			}
		}return (int) v;
